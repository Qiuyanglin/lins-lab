---
title: "林实验室 – 生物医疗集成电路"
type: landing

sections:
  - block: hero
    content:
      title: "生物医疗集成电路<br>与系统实验室（MICAS）"
      text: |
        **复旦大学**  
        从生物信号到芯片 —— 推动下一代生物医疗电子系统。
      image:
        filename: hero_app.jpg
        alt: "生物医疗芯片应用"
    design:
      background:
        gradient_start: "#ffffff"
        gradient_end: "#ffffff"
    typography:
      title:
        size: "2.8rem"
        line_height: 1.1
        weight: 500
      text:
        size: "1.25rem"
        line_height: 1.6

  - block: custom
    content:
      html: |
        <div style="display:flex;gap:32px;align-items:flex-start;flex-wrap:wrap;margin-top:40px;">
          
          <div style="flex:0 0 260px;">
            <img src="https://qiuyanglin.github.io/lins-lab/media/lin.png"
                 alt="林秋阳"
                 style="width:260px;height:auto;border-radius:16px;border:1px solid rgba(0,0,0,.10);box-shadow:0 8px 22px rgba(0,0,0,.10);">
          </div>

          <div style="flex:1;min-width:280px;max-width:760px;line-height:1.75;">
            
            <div style="font-size:1.7rem;font-weight:650;margin-bottom:6px;">
              林秋阳
            </div>

            <div style="color:rgba(0,0,0,.65);margin-bottom:14px;">
              复旦大学 集成电路与微纳电子创新学院<br>
              国家级青年人才 · 上海市青年人才 · 博士生导师 · IEEE Senior Member
            </div>

            <div style="margin-bottom:12px;">
              长期从事<b>模拟与混合信号集成电路设计</b>研究，
              聚焦生物医疗电子系统与高精度数据转换技术。
            </div>

            <div style="margin-bottom:12px;">
              博士毕业于比利时鲁汶大学 MICAS 实验室与欧洲微电子中心 IMEC，
              曾任 IMEC 研究员、高级研究员及模拟电路社区主席。
            </div>

            <div style="margin-bottom:12px;">
              在 ISSCC、VLSI 及 IEEE JSSC、TBCAS 等发表论文 30 余篇，
              授权美国专利 2 项。
            </div>

            <div style="margin-top:12px;">
              <b>链接：</b>
              <a href="https://scholar.google.com/citations?hl=en&user=YajWlVQAAAAJ&view_op=list_works">Google Scholar</a> ·
              <a href="https://orcid.org/0000-0002-7422-5793">ORCID</a> ·
              <a href="mailto:linqiuyang1991@gmail.com">邮箱</a>
            </div>

          </div>
        </div>
---
